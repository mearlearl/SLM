...
L = 0.1;                      % Starting rod length in [m];
...
% Solution Parameters
Nx = 300;                   % Number of rod divisions
Nt = 50;                   % Number of time divisions
K = 30;                     % Number of waves
...
Q2 = J/betahot*(4./(2*mu-sin(2*mu)))./mu./(mu.^2+(delta*L)^2).*((delta*L)*sin(mu)-mu.*cos(mu)+mu.*exp(-delta*L));
%Q2 = 5/beta*(4./(2*mu-sin(2*mu)))./mu./mu.*(-cos(mu)+cos(4/5*mu));
X2  = cell2mat(cellfun(@(ff) ff(x), eigenf,'UniformOutput',false));
X2 = diag(Q2)*X2;
T2 = (1-exp(-betahot*t'*(mu.^2)))';
...
u1 = reshape(sum(Partials1,1),length(x),length(t));

measure3 = measurek(K,u1(:,end),eigenf,x);

Q3 = (4)*(measure3.*mu)./(2*mu-sin(2*mu));
X3  = cell2mat(cellfun(@(ff) ff(x), eigenf,'UniformOutput',false));
X3 = diag(Q3)*X3;
T3 = exp(-betahot*t'*(mu.^2))';
Partials3 = zeros(K,length(x),length(t));

for k = 1:K
    Partials3(k,:,:) = X3(k,:)'*T3(k,:);
end    

u3 = reshape(sum(Partials3,1),length(x),length(t));
